...
void Polygon::andrewHull(Point *s, int n)
{
    ...
    /*Point * finalRoute = new Point[countAbove+countBeneath]; // changes
    Point *res = new Point[countAbove+countBeneath];
    Stack <Point> Route(finalRoute);
    for (int i = 0;i  < countBeneath; i++)
    {
        res[i]= beneath[i];
    }
    for (int i = 0; i < countAbove; i++)
        res[i+countBeneath] = above[i];
    Route.push(res[0]);
    Route.push(res[1]);
    for (int i = 2; i< countAbove+ countBeneath+1; i++)
    {
        Vector v1(Route.NextToTop(),Route.gettop());
        Vector v2(Route.gettop(),res[i % (countAbove+countBeneath)]);
        double mult = v1.PseudoscalarProduct(v2);
        while (mult <= 0 && Route.getsize() > 1)
        {
            Route.pop();
            Vector v1(Route.NextToTop(),Route.gettop());
            Vector v2(Route.gettop(),res[i % (countAbove+countBeneath)]);
        ...
        Route.push(res[i % (countAbove+countBeneath)]);
    }
    Route.pop();*/
     above[countAbove] = pdown; //original code
    ...
    for (int i = 2; i < countBeneath; i++)
    {
        ...
        long long mult = v1.HullCrutchPs(v2);
        while ( mult <= 0  && RouteBeneath.getsize() > 1)
        {
            RouteBeneath.pop();
            Vector v1(RouteBeneath.NextToTop(),RouteBeneath.gettop());
            Vector v2(RouteBeneath.gettop(),beneath[i]);
            mult = v1.HullCrutchPs(v2);
        }
    ...
    for (int i = 2; i < countAbove; i++)
    {
        ...
        long long mult = v3.HullCrutchPs(v4);
        while ( mult <= 0 && RouteAbove.getsize() > 1)
        {
            RouteAbove.pop();
            Vector v3(RouteAbove.NextToTop(),RouteAbove.gettop());
            Vector v4(RouteAbove.gettop(),above[i]);
            mult = v3.HullCrutchPs(v4);
        }
